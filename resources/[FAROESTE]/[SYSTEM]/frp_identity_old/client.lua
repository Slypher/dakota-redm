...
RegisterNetEvent("FRP:IDENTITY:maxChars")
AddEventHandler("FRP:IDENTITY:maxChars",function(source, characterArray)
    SendNUIMessage({type = 2}) -- clear UI
...
RegisterNUICallback(
    ...
    function()
        SetNuiFocus(false, false)
        TriggerEvent("FRP:NOTIFY:Simple","Você atingiu o limite de personagens.",50)
        Wait(2500)
        TriggerEvent("FRP:IDENTITY:maxChars", source)
        return
    end
)
