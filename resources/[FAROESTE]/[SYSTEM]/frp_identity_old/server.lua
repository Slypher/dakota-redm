...
RegisterServerEvent("FRP:IDENTITY:createChar")
AddEventHandler("FRP:IDENTITY:createChar",function()
	local source = source
    local User = API.getUserFromSource(source)
    local Character = User:getCharacter()

	if not API.getUsersByGroup("admin") and parseInt(#persons) >= 1 then
		TriggerClientEvent("FRP:NOTIFY:Simple",source,"Você atingiu o limite de personagens.",5000)
		TriggerClientEvent("FRP:IDENTITY:maxChars",source)
		return
	end

	local newId = 0
	local chars = getCharacter(steam)
