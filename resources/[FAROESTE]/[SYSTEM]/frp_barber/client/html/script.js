...
$(".button-right").on('click', function() {
    ...
    if (facefeature == undefined) {
        $.post('http://frp_barber/' + component, JSON.stringify({ id }));
    } else {
        $.post('http://frp_barber/' + component, JSON.stringify({ facefeature: facefeature, id: id }));
    }
...
$(".button-left").on('click', function() {
    ...
    if (facefeature == undefined) {
        ...
        $.post('http://frp_barber/' + component, JSON.stringify({ facefeature: facefeature, id }));
    }

});
function confirm(){
    $.post('http://frp_barber/Confirm')
    $("#creatormenu").fadeOut(500);
}

function fechar(){
    $.post('http://frp_barber/fechar')
    $("#creatormenu").fadeOut(500);
}

function changeoptions(name) {
    var change = document.getElementById(name).value;
    $.post('http://frp_barber/' + name,  JSON.stringify({ change }));
}
...
window.addEventListener("keydown", throttle(function (ev) {
    ...
    if (_key) {
        $.post('http://frp_barber/rotate', JSON.stringify({key: _key}))
    }
}, 50))
