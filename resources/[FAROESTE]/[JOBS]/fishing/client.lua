local fishingJS = exports['fishing']
...
RegisterNetEvent('net.useBait', function(UsableBait)
    print(UsableBait)
                ...
                if FISHING_GET_MINIGAME_STATE() == 7 then
                    ...
                    if GetGameTimer() >= nextAttTime then
                        ...
                        if probabilidadePuxar > 0.8 or probabilidadePuxar < 0.2 then -- soltar linha
                            ...
                            local r = fishingJS:VERTICAL_PROBE(x, y,  z, 1)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if FISHING_GET_MINIGAME_STATE() == 1 then
			PromptSetActiveGroupThisFrame(fishing_data.prompt_prepare_fishing.group, CreateVarString(10, "LITERAL_STRING", 'Começar a pescar'))
		end	
		
		if FISHING_GET_MINIGAME_STATE() == 6 then
			PromptSetActiveGroupThisFrame(fishing_data.prompt_waiting_hook.group, CreateVarString(10, "LITERAL_STRING", 'Pescando'))
		end

		if FISHING_GET_MINIGAME_STATE() == 7 then
			fishing_data.fish.weight = FISHING_GET_F_(8)
			PromptSetActiveGroupThisFrame(fishing_data.prompt_hook.group, CreateVarString(10, "LITERAL_STRING", 'Pegue o peixe'))
		end
		if FISHING_GET_MINIGAME_STATE() == 12 then

            local modelName = gModelName[GetEntityModel(FISHING_GET_FISH_HANDLE())]

			if modelName then -- Config.Lang.FishName..": "..fishs[GetEntityModel(FISHING_GET_FISH_HANDLE())] .." // "..Config.Lang.FishWeight..": "..string.format("%.2f%%", (fishing_data.fish.weight * 54.25)):gsub("%%", "").."Kg"
                PromptSetActiveGroupThisFrame(fishing_data.prompt_finish.group, CreateVarString(10, "LITERAL_STRING", ('Nome: %s Peso %s'):format(ItemList[modelName].name, ItemList[modelName].weight)))
			end
		end
	end
end)
...
function GET_TASK_FISHING_DATA()
    local r = fishingJS:GET_TASK_FISHING_DATA_EXTRA()
    hasMinigameOn = r[1]
    local outAsInt = r[2]
...
function SET_TASK_FISHING_DATA()
    if fishing_minigame_struct.f_0 ~= nil then
        fishingJS:SET_TASK_FISHING_DATA_EXTRA(fishing_minigame_struct)
    end
end
...
function prepareMyPrompt()
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", 'Preparar para lançar'))
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", 'Lançar linha'))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_prepare_fishing.group)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", 'Reajustar lançamento'))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_waiting_hook.group)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", 'Puxar'))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", 'Devolver'))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_finish.group)
