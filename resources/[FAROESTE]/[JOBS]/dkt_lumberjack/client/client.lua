...
RegisterNetEvent("lumberjack:axechecked")
AddEventHandler("lumberjack:axechecked", function(tree)
    goChop(tree)
end)

RegisterNetEvent("lumberjack:noaxe")
AddEventHandler("lumberjack:noaxe", function()
    active = false
end)

function goChop(tree)
    EquipTool('p_axe02x','Cortar')
    local swing = 0
    ...
    while hastool == true do
        ...
        elseif IsControlJustReleased(0, Config.ChopTreeKey) then
            ...
            Citizen.Wait(1733)
            TriggerServerEvent("lumberjack:addItem")
        end
