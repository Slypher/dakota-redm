            ...
            local rock = DoesObjectOfTypeExistAtCoords(x, y, z, 1.0, GetHashKey(v), true)
                if rock and not InArray(MinedRocks, tostring(v)) then
                    ...
                    if active == false then
                        local MiningGrouName  = CreateVarString(10, 'LITERAL_STRING', "Minerador")
                    ...
                    if PromptHasHoldModeCompleted(MinePrompt) then
                        ...
                        Citizen.Wait(500)
                        TriggerServerEvent("mining:pickaxecheck", tostring(v))
                    end
                else
...
RegisterNetEvent("mining:pickaxechecked")
AddEventHandler("mining:pickaxechecked", function(rock)
    goMine(rock)
end)

RegisterNetEvent("mining:nopickaxe")
AddEventHandler("mining:nopickaxe", function()
    active = false
end)

function goMine(prop)
    EquipTool('p_pickaxe01x', 'Minerar')
    local swing = 0
    ...
    while hastool == true do
        ...
        elseif IsControlJustPressed(0, Config.MineRockKey) then
            ...
            Citizen.Wait(2300)
            TriggerServerEvent('mining:addItem')
        end
