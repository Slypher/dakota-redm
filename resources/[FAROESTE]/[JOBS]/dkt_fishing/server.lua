...
local ItemData = Inventory:addItem(_source, name, meta)
ItemData.AddItem(1)
end)
...
AddEventHandler("redemrp_fishing:Sell", function()
	...
	local User = API.getUserFromSource(_source)
    local Inventory = User:getCharacter():getInventory()
	local ItemData = Inventory:addItem(_source, "smallfish")
	local ItemData2 = Inventory:addItem(_source, "mediumfish")
	local ItemData3 = Inventory:addItem(_source, "largefish")
