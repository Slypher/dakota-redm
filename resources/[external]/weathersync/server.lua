...
RegisterCommand("weather", function(source, args, raw)
    ...
    if not character:hasGroupOrInheritance('admin') then
        return user:notify("error", "Você não tem permissão!")
    end
...
RegisterCommand("time", function(source, args, raw)
	...
	if #args > 0 then
		...
		local d, h, m, s = TimeToDHMS(currentTime)
		printMessage(source, {color = {255, 255, 128}, args = {"error", string.format("%s %.2d:%.2d:%.2d", GetDayOfWeek(d), h, m, s)}})
	end
end, false)
...
RegisterCommand("timescale", function(source, args, raw)
	...
	if args[1] then
		...
		printMessage(source, {color = {255, 255, 128}, args = {"error", currentTimescale}})
	end
end, false)
...
RegisterCommand("syncdelay", function(source, args, raw)
	...
	if args[1] then
		...
		printMessage(source, {color = {255, 255, 128}, args = {"error", SyncDelay}})
	end
end, false)
