...
Citizen.CreateThread(function()
    while true do
        ...
        if (Vdist(coords.x, coords.y, coords.z, 1082.44, -674.86, 111.42) < 2.0) then
            DrawTxt("[~e~G~q~] para usar a Camera.", 0.50, 0.95, 0.7, 0.7, true, 255, 255, 255, 255, true)
...
AddEventHandler('prop:ledger', function() 
    FPrompt("Jogar Fora", 0x3B24C470, false)
...
--Gibana
RegisterNetEvent('prop:gibana')
AddEventHandler('prop:gibana', function() 
    PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
 	ExecuteCommand('close')
    FPrompt("Jogar Fora", 0x3B24C470, false)
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
    local beer = CreateObject(GetHashKey('p_mugCoffee01x'), x, y, z + 0.2, true, true, true)
    local boneIndex = GetEntityBoneIndexByName(ped, "SKEL_R_Finger12")

    TriggerEvent('dangy_stress:modify', -80.0)
...
--Cidrica
RegisterNetEvent('prop:cidrica')
AddEventHandler('prop:cidrica', function() 
    PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
 	ExecuteCommand('close')
    FPrompt("Jogar Fora", 0x3B24C470, false)
    local ped = PlayerPedId()
...
--Tomenta
RegisterNetEvent('prop:tomenta')
AddEventHandler('prop:tomenta', function() 
    PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
 	ExecuteCommand('close')
    FPrompt("Jogar Fora", 0x3B24C470, false)
    local ped = PlayerPedId()
	...
	local health2 = GetEntityHealth(closestPlayerPed)
	local newHealth2 = health2 + 1
    local stamina = GetAttributeCoreValue(closestPlayerPed, 1)
	local newStamina = stamina + 1000
...
-- Cogumelos
--[[RegisterNetEvent('prop:sopacogumelo')
AddEventHandler('prop:sopacogumelo', function() 
    PlaySoundFrontend("Core_Full", "Consumption_Sounds", true, 0)
 	ExecuteCommand('close')
    FPrompt("Jogar Fora", 0x3B24C470, false)
    local ped = PlayerPedId()
    ...
    alcoholDrank = alcoholDrank + 1
    if alcoholDrank == 1 and not drunk then
        drunk = true
        ...
        alcoholDrank = alcoholDrank - 1
        drunk = false
        if alcoholDrank == 0 and not drunk then
        Citizen.InvokeNative(0x4102732DF6B4005F,"idle_a",0, true) --playdrunk
        Citizen.InvokeNative(0x406CCF555B04FAD3 , PlayerPedId(), 0, 0.95)
        end
    end
...
--PocketWatch
RegisterNetEvent('prop:relogiovelho')
AddEventHandler('prop:relogiovelho', function() 
...
AddEventHandler('prop:cigarettes', function() 
    FPrompt("Parar de Fumar", 0x3B24C470, false)
    LMPrompt("Inalar", 0x07B8BEAF, false)
        ...
        while  IsEntityPlayingAnim(ped, "amb_rest@world_human_smoking@male_c@base","base", 3)
            ...
            if stance=="c" then
                if IsControlJustReleased(0, 0x07B8BEAF) then
                    Wait(500)
                    TriggerEvent('dangy_stress:modify', -5.0)
...
AddEventHandler('prop:cigar', function()
    ...
    TriggerEvent('dangy_stress:modify', -11.0)
...
AddEventHandler('prop:pipe', function() 
    FPrompt("Jogar Fora", 0x3B24C470, false)
    LMPrompt("Usar", 0x07B8BEAF, false)
    EPrompt("Pose", 0xD51B784F, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    ...
    while IsEntityPlayingAnim(ped, "amb_rest@world_human_smoking@male_b@base","base", 3) do
        ...
        if IsControlJustReleased(0, 0x07B8BEAF) then
            Wait(500)
            TriggerEvent('dangy_stress:modify', -10.0)
            if IsControlPressed(0, 0x07B8BEAF) then
...
AddEventHandler('prop:fan', function() 
    FPrompt("Guardar", 0x3B24C470, false)
    LMPrompt("Pequeno Aceno", 0x07B8BEAF, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
    ...
    while  IsEntityPlayingAnim(ped, "amb_rest@world_human_fan@female_a@base","base", 3) do
        ...
        if IsControlJustReleased(0, 0x07B8BEAF) then
            Wait(500)
            TriggerEvent('dangy_stress:modify', -7.0)
            if IsControlPressed(0, 0x07B8BEAF) then
...
AddEventHandler('prop:chewingtobacco', function()
    ...
    FPrompt("Parar", 0x3B24C470, false)
    LMPrompt("Faça algo", 0x07B8BEAF, false)
    EPrompt("Pose", 0xD51B784F, false)
    ExecuteCommand('close')
    local ped = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
