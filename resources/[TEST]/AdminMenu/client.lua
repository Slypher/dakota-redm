...
Citizen.CreateThread(function()
 ...
 WarMenu.CreateSubMenu('players', 'admin', 'Joueurs Online')
 WarMenu.CreateSubMenu('self', 'admin', 'Admin')
 WarMenu.CreateSubMenu('playeroptions', 'admin', 'Info Joueurs')
 WarMenu.CreateSubMenu('TP', 'admin', 'Teleport')
 WarMenu.CreateSubMenu('noclip', 'admin', 'Noclip')
 WarMenu.SetSubTitle('admin', 'Created By DevDokus')
	while true do
		Citizen.Wait(0)
		local playerPed = PlayerPedId()
		local coords = GetEntityCoords(playerPed)
		if WarMenu.IsMenuOpened('admin') then
			if WarMenu.MenuButton('Online players', 'players') then end
			...
			elseif WarMenu.Button("Teleported to Player") then
                TriggerEvent("redemrp_notification:start", 'Téléportation sur '   .. GetPlayerName(selectedPlayer) .. '', 5)
                TeleportToPlayer(selectedPlayer);
            elseif WarMenu.Button("Unbug Players (Animations)") then
                TriggerEvent("redemrp_notification:start", 'Tu as resynchronisée '  .. GetPlayerName(selectedPlayer) .. '', 5)
                CancelAnimsAll(selectedPlayer);
            elseif WarMenu.Button("Kick") then
                TriggerEvent("redemrp_notification:start", 'Tu as kick '  .. GetPlayerName(selectedPlayer) .. '', 5)
		    ...
		    elseif WarMenu.IsMenuOpened('self') then
                ...
                TeleportToWaypoint()
		    	elseif WarMenu.Button("Infinite Ammo") then
                InfAmmo = not InfAmmo
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if (IsControlJustReleased(0, keys['DEL']) and IsInputDisabled(2)) or (IsControlJustPressed(0, 0x4AF4D473) and not IsInputDisabled(2)) then
