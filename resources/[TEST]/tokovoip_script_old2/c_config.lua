TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "Cidade",
		TSPassword = "suasenha", -- TeamSpeak channel password (can be empty)
		...
		TSChannelSupport = "https://discord.gg/dakotarp", -- TeamSpeak support channel name displayed on blocking screen
		TSDownload = "https://discord.gg/dakotarp", -- Download link displayed on blocking screen
		...
		localName = "SUACIDADE", -- If set, this name will be used as the user's teamspeak display name
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = escape("[SUACIDADE]"); -- Set the local name
		end);
